...
#ifdef zzMQV70
        case MQOT_TOPIC:
                strcpy(work, "SYSTEM.DEFAULT.TOPIC");
                break;
#endif
        default:
...
void ProcessOAMStringParm(MQCFST *pPCFString, OAMParms *pOAMDefn, MQLONG *iCC) {
        ...
        *iCC = MQCC_OK;
        ...
        {
           int ptr = MQ_ENTITY_NAME_LENGTH;

           pOAMDefn->embeddedSpace = FALSE;
           memset(pOAMDefn->AuthEntity, ' ', MQ_ENTITY_NAME_LENGTH);
           memcpy(pOAMDefn->AuthEntity,
                  pPCFString->String,
                  pPCFString->StringLength);

           /* We need to know whether the AuthEntity contains an embedded */
           /* space character, so calculate that once here. The process   */
           /* is complicated by the fact that the field is space-padded.  */
           while ((--ptr) >= 0)
           {
              if (pOAMDefn->AuthEntity[ptr] != ' ')
              {
                 break;
              }
           }

           /* ptr is now the subscript of the right-most, non-space      */
           /* character. Continue scanning to the left looking for       */
           /* another space.                                             */
           while ((--ptr) >= 0)
           {
              if (pOAMDefn->AuthEntity[ptr] == ' ')
              {
                 pOAMDefn->embeddedSpace = TRUE;
                 break;
              }
           }
           break;
...
void AddToFileOAM(OAMParms *pOAMDefn,
                  int UserVersion,
   ...
   *iCC = MQCC_OK;
   ...
   if (pOAMDefn->ObjectType == MQOT_Q)
   {
      for (qPtr = qListAnchor; qPtr != 0; qPtr = (pqList) qPtr->next)
      {
         ...
         {
            if (qPtr->qType == MQQT_LOCAL &&
                (qPtr->qDefType == MQQDT_TEMPORARY_DYNAMIC ||
                 (qPtr->qDefType == MQQDT_PERMANENT_DYNAMIC && !permDyn)))
            {
               writeDef = 0;
            }
            break;
...
\n#\n");
         ...
         /* There is a defect in MQ such that PCF might return a     */
         /* principal with a completely empty authority list, which  */
         /* results in a syntactically incorrect setmqaut command    */
         /* here. To avoid that, we prefix the faulty command with a */
         /* comment marker.                                          */

         if ((pOAMDefn->ObjectType == MQOT_Q_MGR) &&
             (pOAMDefn->AuthList[0] == -1))
         {
            /* we are going to print an empty authority list ie      */
            /* setmqaut -m <qmn> -t qmgr -p <principal>              */
            if (platform == MQPL_WINDOWS_NT)
            {
               fputs("rem ", fp);
            }
            ...
            fprintf(stderr,
                    "(oam.c) Commenting out setmqaut with empty authority list.\n");
         }
         ...
         if (platform == MQPL_WINDOWS_NT && 
             pOAMDefn->EntityType == MQZAET_GROUP &&
             strstr((char*) &(pOAMDefn->AuthEntity), "@"))
         {
         ...
         switch (pOAMDefn->EntityType)
         {
            ...
            {
               /* If AuthEntity contains a space, then surround  */
               /* the printed value with double quote           */
               /* characters.                                   */

               if (pOAMDefn->embeddedSpace)
               {
                  MQsprintf_X(ParmBuffer,
                              "-p \"%s\" ",
                              pOAMDefn->AuthEntity,
...
/*Don't forget to change the iSeries block as well. */
         ...
         /* There is a defect in MQ such that PCF might return an     */
         /* empty authority list, for the MQMQADM user for the queue  */
         /* manager object which results in a syntactically incorrect */
         /* GRTMQAUT command here. To avoid that, we omit the faulty  */
         /* line.                                                     */

         if ((pOAMDefn->ObjectType == MQOT_Q_MGR) &&
             (pOAMDefn->AuthList[0] == MQAUTH_NONE) &&
             (pOAMDefn->AuthList[1] == -1))
         {
            printLine = FALSE;
            fprintf(stderr,
                    "(oam.c) Commenting out GRTMQAUT with empty authority list.\n");
         }
